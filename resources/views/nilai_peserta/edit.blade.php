...
@section('contents')
        ...
        <form action="{{ route('nilai_peserta.update', $nilaiPeserta->id) }}" method="POST">
            ...
            @method('PUT')
            <div class="mb-3">
                <label for="psrt" class="form-label">Nama Peserta:</label>
                <select name="peserta_id" class="custom-select" required>
                    <option value="" selected disabled>Pilih Nama Peserta</option>
                    @foreach ($psrt as $item)
                        @if(is_object($item)) {{-- Pastikan $item adalah objek --}}
                            <option value="{{ $item->id }}" {{ $item->id == $nilaiPeserta->peserta_id ? 'selected' : '' }}>
                                {{ $item->NIM }} - {{ $item->nama_lengkap }}
                            </option>
                        @else
                            {{-- Handle jika $item bukan objek --}}
                            <option value="{{ $item }}" {{ $item == $nilaiPeserta->peserta_id ? 'selected' : '' }}>
                                {{ $item }} - Nama Peserta Tidak Tersedia
                            </option>
                        @endif
                    @endforeach
                </select>
            ...
            <div class="mb-3">
                ...
                <select name="fasilitator_id" class="custom-select" required>
                    ...
                    @foreach ($fasil as $item)
                        <option value="{{ $item->id }}" {{ $item->id == $nilaiPeserta->fasilitator_id ? 'selected' : '' }}>
                            {{ $item->Nama_Lengkap }}
                        </option>
                    @endforeach
                </select>
