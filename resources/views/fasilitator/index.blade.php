                    ...
                    <div class="card-body">
                        <div class="table-responsive">
                            <div class="card-header d-flex justify-content-between" style="background-color: #ffffff;">
                                ...
                                <div>
                                    <a href="/fasilitator/create" class="btn btn-primary mb-3">
            ...
            <tbody>
                ...
                @forelse ($data as $fasilitator)
                    <tr>
                        ...
                        <td>
                            <a href="/fasilitator/show/{{ Crypt::encryptString($fasilitator->id) }}" class="btn btn-sm btn-info" style="font-size: 14px">
                                <i class="fas fa-database"></i> View
                            </a>
                            @if(auth()->user()->user_role == '3')
                            <a href="/fasilitator/edit/{{ Crypt::encryptString($fasilitator->id) }}" class="btn btn-sm btn-warning" style="font-size: 14px">
                                <i class="fas fa-edit"></i> Update
                            </a>
                            <form action="/fasilitator/delete/{{  $fasilitator->id }}" method="POST" class="d-inline">
                                @csrf
                                <!-- Metode HTTP yang digunakan untuk menghapus data -->
                                @method('DELETE')
                                <button type="submit" class="btn btn-sm btn-danger btn-flat confirm-delete" data-toggle="tooltip" title='Delete' style="font-size: 14px">
